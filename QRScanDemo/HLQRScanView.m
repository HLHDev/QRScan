...
// 手电筒提示文字
@property (nonatomic, strong) UILabel *flashlightLab;
// 手电筒开关
@property (nonatomic, strong) UIButton *flashlightBtn;
// 查看我的二维码
@property (nonatomic, strong) UIButton *checkmyQrCodeBtn;

@end


@implementation HLQRScanView
...
- (void)drawRect:(CGRect)rect{
    ...
    [self addSubview:self.checkmyQrCodeBtn];
}
...
- (UILabel *)tipLab {
    if (!_tipLab) {
        _tipLab = [[UILabel alloc] initWithFrame:CGRectMake(20, self.scanRetangleRect.origin.y + self.scanRetangleRect.size.height + 10, self.frame.size.width - 40, 20)];
        _tipLab.textAlignment = NSTextAlignmentCenter;
        _tipLab.textColor = [UIColor lightGrayColor];
        _tipLab.text = @"放入框内，自动扫描";
        _tipLab.font = [UIFont systemFontOfSize:12];
    }
    return _tipLab;
}

- (UIButton *)checkmyQrCodeBtn {
    if (!_checkmyQrCodeBtn) {
        _checkmyQrCodeBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        _checkmyQrCodeBtn.frame = CGRectMake(self.frame.size.width/2 - 80, self.scanRetangleRect.origin.y + self.scanRetangleRect.size.height + 100, 160, 40);
        [_checkmyQrCodeBtn setTitle:@"查看我的二维码" forState:UIControlStateNormal];
        [_checkmyQrCodeBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        _checkmyQrCodeBtn.backgroundColor = [UIColor blackColor];
        _checkmyQrCodeBtn.layer.cornerRadius = 20;
        _checkmyQrCodeBtn.layer.masksToBounds = YES;
        _checkmyQrCodeBtn.titleLabel.font = [UIFont systemFontOfSize:17];
        [_checkmyQrCodeBtn addTarget:self action:@selector(myCodeClicked) forControlEvents:UIControlEventTouchUpInside];
    }
    return _checkmyQrCodeBtn;
}
